...
export class Home extends Component{
    ...
    async componentDidMount(){
        ...
        await axios.get("/vue/newgames").then(res=>{
            this.setState({
                newgames:res.data.result
            })
        })
    }
    onChange=(e)=>{
        this.refs.web.style.display="none"
        this.refs.fff.style.display="none"
        switch (e.nativeEvent.value) {
            case "手游":
                this.setState({
                    mb:0,
                    mobileGame : this.state.game.filter(item =>item.type===1)
                })
                break;
            case "页游":
                this.setState({
                    mb:1,
                    mobileGame : this.state.game.filter(item =>item.type===2)
                })
                break;
            case "端游":
                this.setState({
                    mb:2,
                    mobileGame : this.state.game.filter(item =>item.type===3)
                })
                break;
            default:
                break;
            ...
            <div style={{marginBottom:"50px"}}>
                <Tabs tabs={tabs}
                    ...
                    <div style={{backgroundColor: '#FAFAFA' ,overflow:"hidden",paddingTop:"20px"}}>
                        ...
                        <p ref="web" style={{color:"#FB7299",fontSize:"16px",margin:"20px auto"}}>点击上面标签查看更多</p>
                        <div ref="fff">
                            {
                                this.state.newgames.map((newgame,index)=>{
                                    return(
                                        <Link key={index} to={`/search/${newgame.title}`}>
                                            <li style={{
                                                listStyle:"none",
                                                float:"left",
                                                width:"100%"
                                            }}>
                                                <img src={newgame.imgurl} alt="" style={{
                                                    width:"92%",
                                                    margin:"0 auto",
                                                    height:"175px",
                                                    borderRadius:"30px"
